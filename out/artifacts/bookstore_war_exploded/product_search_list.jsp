                ...
                <table cellspacing="0" class="listcontent">
                    <tr>
                        <td>
                            <h1>商品目录</h1>
                            <hr />
                            <h1>关键字：${name}</h1>&nbsp;&nbsp;&nbsp;&nbsp;共${pr.totalCount}种商品
                            <hr />
                            ...
                            <div class="pagination">
                                ...
                                <ul>
                                    ...
                                    <c:if test="${pr.currentPage > 1}">
                                        <li><a href="${pageContext.request.contextPath}/product/findProductBySearch?name=${name}&page=${pr.currentPage-1}">上一页</a></li>
                                    </c:if>
                                    <span>第${pr.currentPage}页/共 ${pr.totalPage}页</span>
                                    <!-- 显示下一页 -->
                                    <c:if test="${pr.currentPage < pr.totalPage}">
                                        <li><a href="${pageContext.request.contextPath}/product/findProductBySearch?name=${name}&page=${pr.currentPage+1}">下一页</a></li>
                                    </c:if>
                                    <c:choose>
                                        <c:when test="${pr.totalPage <= 4}">
                                            <c:forEach var="i" begin="1" end="${pr.totalPage}">
                                                <li><a href="${pageContext.request.contextPath}/product/findProductBySearch?name=${name}&page=${i}">${i}</a></li>
                                            </c:forEach>
                                        </c:when>
                                        <c:otherwise>
